...
- Deck creates its six slot roots and 29 library roots lazily, then keeps them stable. Preserve delegated listeners on the slot tray, library, weapon rack, and filters; targeted updates must retain focus and scroll.
- Battles use only the saved six-card order and weapon. Persist partial Deck edits as the optional local profile draft, validate exact uniqueness/known keys before Save, and cancel active search when Deck opens.
