...
The medieval siege-ledger home screen centers the player's local ELO, online matchmaking, and CPU training using bold low-resolution pixel icons, parchment, timber, iron, stone, and heraldic cloth. A compact daily challenge and active loadout sit below the battle controls. Persistent bottom navigation switches between Battle and the full-screen six-card Deck builder at any time; Challenges is visible but disabled for now. The Deck builder has no separate back control, keeps five switchable deck tabs at the top, and automatically saves edits without a Save button.

The quartermaster-style Deck screen keeps a familiar stacked layout: five deck tabs, both illustrated permanent weapons, a 3x2 draw-order tray with positions **#1-#3** labeled **Opening Hand** and **#4-#6** labeled **Next in Cycle**, average-cost/category/readiness summaries, Undo, then the 29-card Armory. Permanent weapons now show the same kind of headline stats as cards. Filled draw-order slots use the same artwork, name, cost, category/type, and first two basic-stat anatomy as Armory cards, and every collapsed card stays the same height. Clicking a card expands an action panel over the content below it: **Info** appears above **Choose Position** or **In Deck #N** in the Armory, while a deck card shows **Info** above **Remove**. The collection uses two columns on phones, three at medium widths, and four on wide layouts; it reflows instead of shrinking text at narrow effective widths or zoom.

Search by card name, Attack/Crew/Magic filters, recommended/cost/name sorting, category headings, Reset, and a no-results recovery action make the full collection browsable without a nested scroll area. **Info** opens full rules and stats without adding, removing, or moving anything.

Expand an Armory card and choose **Choose Position** to open the explicit position picker. A complete deck offers all six positions as atomic replacements; a partial deck offers its valid insertion positions. A card already in the deck reports **In Deck #N** instead of duplicating it. Expand a draw-order card to remove it. Reorder by dragging the card itself with a pointer, briefly holding the card itself on touch, or using the keyboard: Space picks up, Arrow keys or Home/End choose a position, Space drops, and Escape cancels. A single-step **Undo** restores the most recent editor mutation.

Each of the five deck tabs keeps its own cards, permanent weapon, and optional partial draft. Every mutation saves automatically when browser storage is available. A successfully saved complete six-unique-card edit immediately becomes that tab's battle-ready snapshot; an incomplete edit persists as a draft while battles continue using that tab's last complete six-card order and weapon. A successfully saved tab switch also selects the active loadout used by the lobby and the next match. Opening Deck cancels active matchmaking so a queued stale loadout cannot interrupt editing.

If browser storage is unavailable, Deck remains usable for the current page but clearly marks the current edit as memory-only. A failed automatic write cannot change the loadout used by Battle, which remains the last successfully persisted active complete deck. A later successful edit or deck switch persists the pending memory-only edits together.
...
- Deck Info and position-choice sheets trap focus, make the background inert, close or cancel with Escape/backdrop/visible controls, and restore focus to the originating action. Deck membership, replacement/insertion choices, readiness, and drag position remain understandable without color or motion.
