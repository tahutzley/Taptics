...
Use `D:\Taptics` as the only writable checkout for all repository changes, generated files, commands, and verification. Before acting, verify the resolved repository root is exactly `D:\Taptics`. If a task opens any other copy, including a OneDrive checkout, do not modify it; switch the tool working directory to `D:\Taptics`. Do not push and pull merely to transfer work between local copies unless the user explicitly requests it.
